...
from src.agent._base import build_openrouter_llm, format_question, load_model_spec, load_prompt_bundle
...
PROMPT_PATH = (
    ...
    / "prompt"
    / "marketing_contents"
    / "MarketingVideoGeneratorAgent.yml"
)
MODEL_SPEC_PATH = (
    Path(__file__).resolve().parents[2]
    / "resources"
...
class MarketingVideoGeneratorAgent:
    def __init__(self) -> None:
        self.prompt = load_prompt_bundle(PROMPT_PATH)
        self.model_spec = load_model_spec(MODEL_SPEC_PATH, kind="video_gen")
